...
const Homelist = ({title, FETCHURL}) => {
  ...
  return (
    ...
    <MdChevronLeft size={40} onClick={scrollLeft} className='bg-gray-200 rounded-full text-blue-300 opacity-50'/>
...
{like ? <FaHeart   className='text-red-600 font-bold left-8 top-4 '/>: <FaRegHeart  className='text-red-600 font-bold  top-8'/>}
        ...
        {/* </div> */}
        <MdChevronRight onClick={scrollright} size={40} className='bg-gray-200 rounded-full text-blue-300 opacity-50 right-0 items-center hidden' />
      </div>
    </>
  );
}
